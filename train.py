...
result = model.fit(np.asarray(X_train).astype('float32'), np.asarray(Y_train).astype('float32'), epochs=20, batch_size=10)
...
loss, acc = model.evaluate(np.asarray(X_test).astype('float32'), np.asarray(Y_test).astype('float32'))
